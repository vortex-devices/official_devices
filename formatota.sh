...
echo -e "${RED}./formatota.sh ${BLUE}out/target/product/jason/Vortex-OS_platina-9.0-20190411-2213-OFFICIAL.zip ${GRAY}help"
exit
fi
...
size=$(cat $1 | wc -c)
device=$(sed 's/.*Vortex-OS_\(.*\)-9.*/\1/' <<< "${name}")
...
echo -e "${RED}         \x22website_url\x22${NC}:${BLUE}\x22https://vortex-os.org\x22,";
echo -e "${RED}         \x22news_url\x22${NC}:${BLUE}\x22https://t.me/VortexOS\x22,";
